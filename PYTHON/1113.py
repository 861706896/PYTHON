...
CASE_BACKGROUND = """
【案件背景】：
2025年11月19日下午3点15分，302路公交车行驶至"西湖文化广场"站时，一名女大学生（受害人）报警称随身携带的双肩包被盗，包内有笔记本电脑（贴有"中国美术学院"校徽贴纸）、学生证及现金500元。司机立即锁闭车门，等待警察到场。

经初步调查，受害人坐在车厢中部靠窗位置，于"武林广场"站上车，失窃发生在"西湖文化广场"站停车前2分钟。当时车厢内乘客较多，受害人因低头看手机未察觉包被拉开。警察到场后，根据受害人描述及监控初步锁定4名重点嫌疑人（均在受害人附近区域）。

请通过盘问嫌疑人，结合他们的言行破绽，找出真正的小偷！
"""

# 新增：嫌疑人当日具体动线（每个角色新增"schedule"字段）
SUSPECTS = {
    "1号乘客（穿蓝色外套的中年男性）": {
...
class DetectiveGame:
    ...
    def create_ui(self):
        ...
        self.add_chat("系统", CASE_BACKGROUND)  # 插入详细案件背景
        ...
        self.question_entry.bind("<Return>", self.handle_enter_key)  # Enter键发送
    ...
    def accuse_thief(self):
        suspect_name = self.current_suspect
        if not suspect_name:
            messagebox.showinfo("提示", "请先选择一名嫌疑人！")
            return
        # 获取当前嫌疑人是否为小偷
        is_thief = SUSPECTS[suspect_name]["role_type"] == "thief"
        
        # 构建结局弹窗内容（含证据链展示）
        if is_thief:
            # 小偷结局：展示作案动线+关键证据
            thief_schedule = "\n".join([f"• {step}" for step in SUSPECTS[suspect_name]["schedule"]])
            evidence = [
            ...
            result_text = f"✅ **指认成功！{suspect_name}就是小偷！**\n\n"
            result_text += "🔍 **作案全过程还原**：\n" + thief_schedule + "\n\n"
            result_text += "📌 **关键证据链**：\n" + "\n".join(evidence)
        else:
            # 无辜结局：提示重新判断
            result_text = f"❌ **指认错误！{suspect_name}是无辜的。**\n\n"
            result_text += "💡 提示：请重新盘问其他嫌疑人，注意他们回答中的矛盾点（如时间、位置、行为细节）！"
        
        # 显示结局弹窗
        messagebox.showinfo("指认结果", result_text)
        if is_thief:
            # 破案后询问是否重新开始
            if messagebox.askyesno("游戏结束", "恭喜你成功破案！是否重新开始游戏？"):
                self.reset_game()
            else:
                self.root.quit()

    def reset_game(self):
        """重置游戏状态，重新随机选择小偷（此处简化为固定1号为小偷，实际可随机）"""
        self.messages = []
        self.current_suspect = random.choice(list(SUSPECTS.keys()))
        self.highlight_current_suspect()
        self.chat_log.config(state=tk.NORMAL)
        self.chat_log.delete(1.0, tk.END)
        self.add_chat("系统", CASE_BACKGROUND)
        self.add_chat("系统", f"当前盘问对象：{self.current_suspect}（点击左侧名字可切换）")
        self.chat_log.config(state=tk.DISABLED)

    def highlight_current_suspect(self):
        """高亮当前选中的嫌疑人按钮"""
        for suspect, btn in self.suspect_buttons.items():
            if suspect == self.current_suspect:
                btn.config(bg="#3498db", fg="white", font=("SimHei", 11, "bold"))
            else:
                btn.config(bg="#f7f9fc", fg="#2c3e50", font=("SimHei", 11))
